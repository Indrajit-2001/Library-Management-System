...
Label(top, font=("Arial","23"), text="Library management app").place(x=90,y=0)
...
def addMember():
	...
	Label(gtk1, height="200", width="2000", background="goldenrod1").place(x=0,y=0)
	Label(gtk1, text="Enter name", font=("Bahnschrift","15"), background="goldenrod1").place(x=20,y=20)
	Label(gtk1, text="Enter phone number", font=("Bahnschrift","15"), background="goldenrod1").place(x=20,y=70)
	def addMemberFunc():
		e = rndm() 
		if w[0].get() == "":
			msg.showerror("Add Member","The data is not filled")
			gtk1.destroy()
			return 
		os.mkdir("members/"+e)
...
def addBook():
	...
	Label(gtk2, height="200", width="2000", background="bisque1").place(x=0,y=0)
	Label(gtk2, text="Enter name", font=("Bahnschrift","15"), background="bisque1").place(x=20,y=20)
	Label(gtk2, text="Enter author", font=("Bahnschrift","15"), background="bisque1").place(x=20,y=70)
	def addBookFunc():
		e = rndm()
		if w[0].get() == "":
			msg.showerror("Add Member","The data is not filled")
...
def removeDue():
	...
	if w=="":
		msg.showerror("Error","Enter the data")
		return
	if (w in os.listdir("members")):
	...
	elif (w not in os.listdir("members")):
...
def getInfo():
	...
	def instagram():
		webbrowser.open("https://www.instagram.com/indrajit7074/")
	def Linkedin():
		webbrowser.open("https://www.linkedin.com/in/indrajit-dutta-70bab0204/")
	def github():
		webbrowser.open("https://github.com/70744034000")
	Label(gtk3,background="black", height="100", width="300").place(x=0,y=0)
	Label(gtk3,background="black" ,text="This app is made by Indraj!t Dutta",foreground="white", font=("Bahnschrift","15")).place(x=0,y=0)
	Button(gtk3, text="Instagram", background="pink", foreground="black" ,font=("Bahnschrift","18"), command=instagram).place(x=10,y=50)
	Button(gtk3, text="Linked-in", background="bisque1", foreground="brown" ,font=("Bahnschrift","18"), command=Linkedin).place(x=10,y=110)
	Button(gtk3, text="Github", background="brown", foreground="white" ,font=("Bahnschrift","18"), command=github).place(x=10,y=170)
	gtk3.mainloop()
...
def assignBook():
	...
	Label(gtk3, height="200", width="2000", background="goldenrod1").place(x=0,y=0)
	Label(gtk3, text="Enter member id", font=("Bahnschrift","15"), background="goldenrod1").place(x=20,y=20)
	Label(gtk3, text="Enter book id", font=("Bahnschrift","15"), background="goldenrod1").place(x=20,y=70)
	def assignBookFunc():
		e=[w[0].get(), w[1].get()]
		if e[0]=="":
			msg.showerror("Error","No data Entered")
			gtk3.destroy()
			return
		if e[0] in os.listdir("members") :
...
def listBooks():
	...
	def listBks():
		...
		if (len(lb) < dfk[1]):
			...
			if dfk[0] > 0:
				Button(gtk4,text="Back", font=("Bahnschrift","18"), foreground="white", background="SkyBlue2", command=prev).place(x=120,y=510)
		else:
			...
			if dfk[0] > 0:
				Button(gtk4,text="Back", font=("Bahnschrift","18"), foreground="white", background="SkyBlue2", command=prev).place(x=120,y=510)
			Button(gtk4,text="Next", font=("Bahnschrift","18"), foreground="white", background="SkyBlue2", command=prec).place(x=300,y=510)
	listBks()
	gtk4.mainloop()
...
def listMembers():
	...
	def listMbr():
		...
		if (len(lb) < dfk[1]):
			...
			if dfk[0] > 0:
				Button(gtk5,text="Back", font=("Bahnschrift","18"), foreground="white", background="SkyBlue2", command=prev).place(x=120,y=510)
		else:
			for i in range(dfk[0],dfk[1]):
				if (open("members/"+lb[i]+"/book","r").read() == "0"):
					gh="AV"
			...
			if dfk[0] > 0:
				Button(gtk5,text="Back", font=("Bahnschrift","18"), foreground="white", background="SkyBlue2", command=prev).place(x=120,y=510)
			Button(gtk5,text="Next", font=("Bahnschrift","18"), foreground="white", background="SkyBlue2", command=prec).place(x=300,y=510)
	listMbr()
	gtk5.mainloop()



Button(top, text="Add member", background="Blue", foreground="white", font=("Bahnschrift","15"), command=addMember).place(x=20,y=50)
Button(top, text="Add book", background="Blue", foreground="white", font=("Bahnschrift","15"), command=addBook).place(x=20,y=110)
Button(top, text="Assign book", background="Blue", foreground="white", font=("Bahnschrift","15"), command=assignBook).place(x=20,y=170)
Button(top, text="Remove member", background="Blue", foreground="white", font=("Bahnschrift","15"), command=removeMember).place(x=320,y=50)
Button(top, text="Remove book", background="Blue", foreground="white", font=("Bahnschrift","15"), command=removeBook).place(x=320,y=110)
Button(top, text="Get info", background="Blue", foreground="white", font=("Bahnschrift","15"), command=getInfo).place(x=198,y=290)
Button(top, text="List members", background="Blue", foreground="white", font=("Bahnschrift","15"),command=listMembers).place(x=20,y=230)
Button(top, text="List books", background="Blue", foreground="white", font=("Bahnschrift","15"),command=listBooks).place(x=320,y=230)
Button(top, text="Remove due", background="Blue", foreground="white", font=("Bahnschrift","15"), command=removeDue).place(x=320,y=170)
